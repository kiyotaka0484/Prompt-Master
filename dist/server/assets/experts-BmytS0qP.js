//#region src/lib/experts.ts
var BASE_INSTRUCTIONS = `You are Prompt Master — an expert consultant whose only job is to help a beginner craft a high-quality, AI-ready master prompt that they can paste into ChatGPT, Gemini, or Claude.
...
function buildPlannerContext(expert, goal, filled) {
	const known = [];
	const missingByP = {
		critical: [],
		important: [],
	...
	for (const s of expert.slots) {
		const v = filled[s.id];
		if (v && v.trim()) known.push(`- ${s.id} (${s.label}): ${v.trim().slice(0, 140)}`);
		else missingByP[s.priority].push(`- ${s.id} — ${s.label} — why: ${s.why}`);
	}
	const fmt = (rows) => rows.length ? rows.join("\n") : "(none)";
...
var TRIAGE_PROMPT = `You are Prompt Master's intake consultant. The user has NOT yet stated a real goal — their last message is a greeting, an emotional opener, a confused message, or otherwise too vague to assign a specialist.
...
var VAGUE_INPUTS = /* @__PURE__ */ new Set([
	"hi",
	"hii",
	"hiii",
	"hello",
...
function detectExpert(text) {
	...
	for (const k of Object.keys(buckets)) for (const kw of buckets[k]) if (t.includes(kw)) score[k] += kw.length > 5 ? 2 : 1;
	let best = "business";
	let bestScore = 0;
	for (const k of Object.keys(score)) if (score[k] > bestScore) {
		bestScore = score[k];
		best = k;
	}
...
//#endregion
export { detectExpert as a, buildPlannerContext as i, EXPERT_LIST as n, TRIAGE_PROMPT as r, EXPERTS as t };
