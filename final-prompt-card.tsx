...
const MODELS: {
  ...
  { key: "chatgpt", label: "ChatGPT", emoji: "🟢", accent: "text-emerald-400", tint: "border-emerald-500/40" },
  { key: "claude",  label: "Claude",  emoji: "🟠", accent: "text-orange-400",  tint: "border-orange-500/40"  },
  { key: "gemini",  label: "Gemini",  emoji: "🔷", accent: "text-sky-400",     tint: "border-sky-500/40"     },
];

export function FinalPromptCard({ prompts, onRegenerate, regenerating }: Props) {
  ...
  return (
    <div className="my-3 overflow-hidden rounded-xl border border-primary/40 bg-gradient-to-br from-primary/10 via-background to-background shadow-[0_0_40px_-15px_color-mix(in_oklch,var(--primary)_60%,transparent)]">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as ModelKey)}>
        ...
        {MODELS.map((m) => (
          <TabsContent key={m.key} value={m.key} className="m-0">
            <div className={"flex items-center justify-between border-b border-border/40 bg-card/20 px-4 py-2 " + m.tint}>
              <div className={"text-[11px] font-semibold uppercase tracking-wider " + m.accent}>
      ...
      <div className="flex items-start gap-2 border-t border-border/40 bg-muted/20 px-4 py-3 text-[12px] leading-relaxed text-muted-foreground">
        ...
        <div>
          <div className="mb-0.5 font-medium text-foreground/80">Why are these prompts different?</div>
          Each model thinks differently.{" "}
          <span className="text-emerald-400">ChatGPT</span> responds best to explicit roles and structured formatting,{" "}
          <span className="text-orange-400">Claude</span> shines with conversational context and thoughtful reasoning, and{" "}
          <span className="text-sky-400">Gemini</span> is sharpest with direct, labeled task instructions. Same goal, three native styles.
        </div>
      </div>
    </div>
  );
