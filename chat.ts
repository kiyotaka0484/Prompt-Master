...
import { EXPERTS, TRIAGE_PROMPT, buildPlannerContext, detectExpert, type ExpertId } from "@/lib/experts";
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const goalIndex = expert
          ? uiMessages.findIndex((m) => m.role === "user" && detectExpert(messageTextFromUi(m)))
          : -1;
