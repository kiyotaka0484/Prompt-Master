...
const BUSINESS_SLOTS: Slot[] = [
  { id: "country",    label: "Country / Market",   priority: "critical",  why: "Markets, regulations, and realistic strategies depend heavily on where you operate." },
  { id: "budget",     label: "Starting Budget",    priority: "critical",  why: "Your budget filters which business models, tools, and growth tactics are actually viable." },
  { id: "audience",   label: "Target Customer",    priority: "critical",  why: "Who you sell to drives positioning, pricing, and channels in the final prompt." },
  { id: "model",      label: "Revenue Model",      priority: "critical",  why: "Service vs product vs SaaS vs e-commerce changes the entire playbook the AI will recommend." },
  { id: "skills",     label: "Existing Skills",    priority: "important", why: "Lets the AI lean on what you already know instead of suggesting a steep learning curve." },
  { id: "experience", label: "Experience Level",   priority: "important", why: "Calibrates how basic or advanced the recommended steps should be." },
  { id: "time",       label: "Time Available",     priority: "important", why: "Hours per week decides whether this is a side project or a full launch plan." },
  { id: "timeline",   label: "Timeline",           priority: "important", why: "Sets realistic milestones and urgency for the final plan." },
  { id: "risk",       label: "Risk Tolerance",     priority: "optional", why: "Helps the AI pick safer or more aggressive strategies you'll be comfortable with." },
];

const WEBSITE_SLOTS: Slot[] = [
  { id: "purpose",    label: "Purpose",            priority: "critical",  why: "The site's main goal (showcase, sell, book, teach) shapes every page and feature recommendation." },
  { id: "audience",   label: "Target Audience",    priority: "critical",  why: "Who visits the site drives tone, layout, and content priorities." },
  { id: "features",   label: "Key Features",       priority: "critical",  why: "Listing the must-have features defines scope and complexity in the final prompt." },
  { id: "design",     label: "Design Style",       priority: "critical",  why: "Visual direction (minimal, bold, playful, premium) keeps the design coherent." },
  { id: "type",       label: "Site / App Type",    priority: "important", why: "Portfolio vs landing vs SaaS vs blog changes the structure entirely." },
  { id: "profession", label: "Profession / Niche", priority: "important", why: "Lets the AI suggest profession-specific sections instead of generic ones." },
  { id: "pages",      label: "Pages & Sections",   priority: "important", why: "Defines the sitemap so the AI's plan matches what you actually need." },
  { id: "experience", label: "Technical Experience", priority: "important", why: "Sets whether the prompt should target no-code, low-code, or full-code tooling." },
  { id: "hosting",    label: "Hosting & Domain",   priority: "optional", why: "Helps the AI recommend a realistic launch path." },
  { id: "budget",     label: "Budget",             priority: "optional", why: "Filters out tooling and services you can't justify." },
];

const STUDY_SLOTS: Slot[] = [
  { id: "subject",     label: "Subject",             priority: "critical",  why: "The exact subject defines the entire roadmap and resources." },
  { id: "level",       label: "Current Level",       priority: "critical",  why: "Calibrates where the learning plan starts so it isn't too easy or too hard." },
  { id: "deadline",    label: "Deadline",            priority: "critical",  why: "Sets the pace and how aggressive the weekly plan should be." },
  { id: "time",        label: "Weekly Study Time",   priority: "critical",  why: "Hours per week determines what's realistic to cover before the deadline." },
  { id: "goal_detail", label: "End Goal",            priority: "important", why: "Job, exam, or portfolio piece changes which topics matter most." },
  { id: "style",       label: "Learning Style",      priority: "important", why: "Lets the AI mix videos, projects, reading, and practice the way you learn best." },
  { id: "background",  label: "Prior Experience",    priority: "important", why: "Helps skip topics you already know and avoid wasted weeks." },
  { id: "language",    label: "Language",            priority: "optional", why: "Picks resources in a language you're comfortable with." },
  { id: "budget",      label: "Budget",              priority: "optional", why: "Decides whether free or paid courses are recommended." },
];

const CREATOR_SLOTS: Slot[] = [
  { id: "niche",        label: "Niche",                  priority: "critical",  why: "A clear niche is the single biggest factor in growth — it shapes every content recommendation." },
  { id: "platform",     label: "Platform",               priority: "critical",  why: "YouTube vs TikTok vs Instagram changes format, length, and strategy completely." },
  { id: "format",       label: "Content Style (face / faceless)", priority: "critical", why: "Facecam vs faceless decides production setup and content ideas." },
  { id: "frequency",    label: "Upload Frequency",       priority: "critical",  why: "How often you can publish drives the realistic growth plan." },
  { id: "audience",     label: "Target Audience",        priority: "important", why: "Knowing who you're for sharpens hooks, titles, and topic choices." },
  { id: "length",       label: "Shorts vs Long-form",    priority: "important", why: "Format length controls the editing workload and growth path." },
  { id: "monetization", label: "Monetization Goal",      priority: "important", why: "Ad revenue, sponsorships, products, or coaching change content strategy." },
  { id: "time",         label: "Hours per Week",         priority: "important", why: "Sets realistic production volume in the final plan." },
  { id: "language",     label: "Language",               priority: "optional", why: "Defines the audience pool and competition level." },
  { id: "equipment",    label: "Equipment",              priority: "optional", why: "Helps tailor the production setup advice." },
];

function priorityLabel(p: SlotPriority): string {
  return p === "critical" ? "CRITICAL" : p === "important" ? "IMPORTANT" : "OPTIONAL";
}

function slotsBlock(slots: Slot[]): string {
  return `\n\n# Slot IDs you may use in <!--slot:...--> tags (with priority + why)\n${slots
    .map((s) => `- ${s.id} — ${s.label} [${priorityLabel(s.priority)}] — why: ${s.why}`)
...
export const EXPERTS: Record<ExpertId, Expert> = {
  business: {
    ...
    description: "Find the right business idea and first steps for your skills, budget, and time.",
    emoji: "💼",
    accent: "var(--accent-violet)",
    examples: ["I want to start a business", "Side income ideas", "Launch a startup"],
  ...
  website: {
    ...
    description: "Plan the right type of site or app — features, pages, and tech — even with zero coding.",
    emoji: "🧩",
  ...
  study: {
    ...
    description: "Get a clear roadmap to learn anything — adapted to your level, time, and goals.",
    emoji: "📚",
  ...
  creator: {
    ...
    description: "Find your niche, format, and content strategy for any platform.",
    emoji: "🎬",
...
export function buildPlannerContext(
  ...
  const known: string[] = [];
  const missingByP: Record<SlotPriority, string[]> = { critical: [], important: [], optional: [] };
...
const VAGUE_INPUTS = new Set([
  ...
  "yo", "yoo", "yooo", "sup", "wassup", "whatsup", "whats up", "what's up",
  "bro", "bruh", "bruv", "dude", "yo bro", "hey bro", "hi bro", "sup bro",
  "gm", "good morning", "morning", "good afternoon", "afternoon",
  "good evening", "evening", "good night", "gn",
  // help / confusion / fillers
  "help", "idk", "i don't know", "i dont know", "dunno", "not sure",
  "maybe", "nothing", "ok", "okay", "k", "kk",
  "what", "what?", "?", "??", "...",
  "start", "go", "test", "testing", "hmm", "hmmm", "huh",
  // bangla / banglish greetings
  "salam", "salaam", "assalamualaikum", "assalamu alaikum", "walaikum assalam",
  "namaste", "namaskar", "hola",
  "vai", "bhai", "apu", "apa",
  "kemon acho", "kemon achen", "ki khobor", "ki obostha", "ki obostha vai",
]);
...
export function detectExpert(text: string): ExpertId | null {
  ...
  const buckets: Record<ExpertId, string[]> = {
    business: [
      "business", "startup", "company", "saas", "ecommerce", "e-commerce", "money",
      "income", "side hustle", "freelance", "agency", "sell", "product", "service",
      "shop", "store", "dropship", "invest", "entrepreneur", "client", "profit", "revenue",
    ],
    website: [
      "website", "site", "app", "application", "build", "code", "develop", "platform",
      "landing page", "portfolio site", "web", "react", "next", "frontend", "backend",
      "api", "database", "saas product", "mobile app", "ios", "android",
    ],
    study: [
      "learn", "study", "course", "tutorial", "school", "university", "exam", "skill",
      "language", "python", "javascript", "math", "english", "spanish", "guitar",
      "drawing", "improve", "career", "certification", "roadmap",
    ],
    creator: [
      "youtube", "tiktok", "instagram", "twitch", "podcast", "channel", "content",
      "creator", "video", "short", "shorts", "reels", "stream", "audience", "followers",
      "subscribers", "viral", "post", "blog", "newsletter",
    ],
  };
